...
const HistoriaClinicaForm = () => {
  ...
  const { id } = useParams(); // <-- Esto detecta si estás editando
  ...
  const fetchHistoria = async () => {
    if (!id) return; // solo si hay ID (modo edición)

    try {
      const token = localStorage.getItem("token");
      const res = await axios.get(`http://localhost:3000/api/historia-completa/${id}`, {
      ...
      setFormData({
        ...
        fecha_evaluacion: formatDate(historia.fecha_evaluacion),
      ...
      setAntecedentes({
        ...antecedentes,
        vacunacion_completa: antecedentes.vacunacion_completa || false
      });

      setEvaluacionPostural(evaluacionPostural || {});
      setFuerzaMuscular(fuerzaMuscular || []);
      setInformeFinal({
        ...informeFinal,
        fecha_informe: formatDate(informeFinal.fecha_informe)
      });
      setSeguimientos(
        (seguimientos || []).map(s => ({
          ...s,
